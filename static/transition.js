...
(function () {
  ...
  loader.innerHTML = '<div class="pl-spinner"><div class="pl-blob"></div><div class="pl-text">OASIS-X</div></div>';
  loader.style.cssText = 'position:fixed;inset:0;z-index:99999;background:#030a12;display:none;align-items:center;justify-content:center;opacity:0;transition:opacity 0.3s ease;flex-direction:column;gap:20px;';
  ...
  // Intercept internal link clicks for PAGE NAVIGATIONS only
  document.addEventListener('click', function (e) {
    var link = e.target.closest('a');
    if (!link) return;
    var href = link.getAttribute('href');
    if (!href) return;
    // Skip: external, hash-only, javascript:, mailto:, target=_blank, modifier keys
    if (href.startsWith('http') || href.startsWith('mailto:') || href.startsWith('javascript:')) return;
    if (href.startsWith('#')) return;
    if (link.target === '_blank') return;
    if (e.ctrlKey || e.metaKey || e.shiftKey) return;
    // Skip same-page anchors (e.g. /#features, /?q=1#section)
    var destPath = href.split('#')[0].split('?')[0];
    var currentPath = window.location.pathname;
    if (destPath === '' || destPath === currentPath || destPath === '.') return;
    e.preventDefault();
    showLoader(href);
  });
})();
